...
public class GetScoreboardTool implements Tool {
    ...
    private JsonObject listObjectives(ServerScoreboard sb) {
        ...
        for (Objective o : sb.getObjectives()) {
            ...
            int total = sb.listPlayerScores(o).size();
    ...
    private JsonObject readObjective(ServerScoreboard sb, JsonObject args) throws ToolException {
        ...
        for (PlayerScoreEntry entry : sb.listPlayerScores(obj)) {
            ...
            row.addProperty("holder", entry.owner());
            row.addProperty("score", entry.value());
            var info = sb.getPlayerScoreInfo(ScoreHolder.forNameOnly(entry.owner()), obj);
            row.addProperty("locked", info != null && info.isLocked());
            scores.add(row);
        }
        JsonObject r = new JsonObject();
