...
public class GetItemInfoTool implements Tool {
    ...
    public JsonObject invoke(JsonObject args, ClientSession session) throws ToolException {
        ...
        if (foundItem) {
            ...
            r.addProperty("max_stack", stack.getMaxStackSize());
            r.addProperty("max_damage", stack.getMaxDamage());
            ...
            r.addProperty("fire_resistant", stack.has(DataComponents.FIRE_RESISTANT));
            r.addProperty("rarity", stack.getRarity().name().toLowerCase(java.util.Locale.ROOT));
            // Edibility
            var props = stack.getFoodProperties(null);
            if (props != null) {
                JsonObject food = new JsonObject();
                food.addProperty("nutrition", props.nutrition());
                food.addProperty("saturation_modifier", props.saturation());
                food.addProperty("can_always_eat", props.canAlwaysEat());
                food.addProperty("fast_food", props.eatSeconds() < 1.6F);
                food.addProperty("meat", stack.is(ItemTags.MEAT));
                r.add("food", food);
            }
            // Fuel value (Forge's hook)
            try {
                int burn = stack.getItem().getBurnTime(stack, null);
