...
public final class McpHttpServer implements HttpHandler {
    ...
    public McpHttpServer(Snapshot cfg, RequestDispatcher dispatcher) {
        ...
        String publicHost = cfg.mcpPublicHost();
        ...
        this.pairingActive = IssuedTokens.current().size() == 0;
        rotatePairCode();
    }
    ...
    public void start() throws IOException {
        ...
        server.start();
        if (pairingActive) schedulePairRefresh();
    }
    ...
    public synchronized String setupLink() {
        return publicPairUrl + "/setup/" + pairSetupId;
    }
    ...
    public synchronized boolean pairingNeeded() {
        return IssuedTokens.current().size() == 0;
    }
    ...
    public SetupLink refreshSetupLink(IssuedTokens.Tier tier) {
        ...
        AgentLinkMod.LOG.info("agent-link local setup endpoint manually refreshed (send this to your AI agent, one use, expires at {}, tier={}): {}",
    ...
    private void handleSetup(HttpExchange ex) throws IOException {
        try (ex) {
            if (!"GET".equalsIgnoreCase(ex.getRequestMethod())) {
                ex.getResponseHeaders().add("Allow", "GET");
                writeStatus(ex, 405, "Only GET is supported");
                return;
            }
            if (!originAllowed(firstHeader(ex, "Origin"))) {
                writeStatus(ex, 403, "Origin not allowed");
                return;
            }
            ...
            writeJson(ex, status, descriptor);
        }
    }
    ...
    private synchronized JsonObject setupDescriptor() {
        ...
        pairRequest.addProperty("url", publicPairUrl);
        ...
        result.addProperty("mcp_url", publicMcpUrl);
        result.addProperty("pair_url", publicPairUrl);
        ...
        result.addProperty("allow_remote", cfg.allowRemote());
    ...
    private void refreshPairCodeIfNeeded() {
        ...
        long expires;
        synchronized (this) {
            if (pairConsumed || server == null || !pairingActive) return;
            rotatePairCode();
            link = setupLink();
            expires = pairExpiresAtMs;
        }
        AgentLinkMod.LOG.info("agent-link local setup endpoint refreshed (send this to your AI agent, one use, expires at {}): {}",
    ...
    private static void addNoStoreHeaders(HttpExchange ex) {
